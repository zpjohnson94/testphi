...
export const Route = createFileRoute("/profile")({
  head: () => ({ meta: [{ title: "Profile — TestPhi" }] }),
  component: Profile,
});
      ...
      <div className="topo-bg topo-dim min-h-screen">
        <div className="mx-auto max-w-2xl p-5 space-y-5 animate-fade-up">
          ...
          >
            <button
              onClick={startEdit}
              className="size-24 rounded-3xl flex items-center justify-center transition-transform hover:scale-105"
          ...
          {editing && (
            ...
            >
              ...
              <div className="mt-3 flex justify-center">
                <Avatar config={draft} size={120} animate />
              </div>
              ...
              <div className="mt-2 grid grid-cols-6 gap-2">
                {ANIMALS.map((a) => (
                  <button
                    key={a.id}
                    onClick={() => setDraft({ ...draft, animal: a.id as AnimalId })}
                    className="aspect-square rounded-2xl flex items-center justify-center"
                    style={{
                      background:
                        draft.animal === a.id ? "var(--violet-deep)" : "rgba(74,6,136,0.06)",
                      border:
                        draft.animal === a.id ? "2px solid var(--neon)" : "2px solid transparent",
                    }}
                  >
                    <Avatar
                      config={{ ...draft, animal: a.id as AnimalId, accessory: "none" }}
                      size={32}
                    />
                  </button>
                ))}
                ...
                {COLOR_SWATCHES.map((c) => (
                  <button
                    ...
                    style={{
                      background: c,
                      border:
                        draft.color === c ? "3px solid var(--ink)" : "2px solid rgba(0,0,0,0.15)",
                  ...
                  return (
                    ...
                    >
                      <div className="aspect-square flex items-center justify-center">
                        {unlocked ? (
                          <Avatar config={{ ...draft, accessory: a.id as AccessoryId }} size={44} />
                        ) : (
                          <Lock className="size-5" style={{ color: "#5a4a72" }} />
                        )}
                      </div>
                      <div
          ...
          >
            ...
            <div className="space-y-3">
              {SKILLS.map((s) => {
                ...
                const color =
                  m >= 70 ? "var(--volt)" : m >= 40 ? "var(--spark)" : "var(--destructive)";
                return (
                  <div key={s.id}>
                    <div
                    ...
                    >
                      <div
                        ...
                        style={{ width: `${m}%`, background: color }}
                      />
                    </div>
                  </div>
                );
