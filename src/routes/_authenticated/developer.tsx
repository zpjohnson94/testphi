      ...
      <div className="topo-bg topo-dim min-h-screen">
        <div className="mx-auto max-w-2xl p-5 space-y-4 pb-28">
          <div className="flex items-center gap-2">
            ...
            <h1 className="display text-2xl" style={{ color: "var(--destructive)" }}>
              Developer mode
            </h1>
          </div>
...
function DeveloperMenu({ state }: { state: FreeState }) {
  ...
  return (
    <div
      ...
      style={{ background: "rgba(255,77,109,0.06)", border: "1.5px dashed rgba(255,77,109,0.55)" }}
    >
      <div className="flex items-center gap-2 px-5 py-4">
        <Wrench className="size-4" style={{ color: "var(--destructive)" }} />
        <span className="display text-base" style={{ color: "var(--destructive)" }}>
          Developer mode
        </span>
      </div>
      ...
      <div
        ...
        style={{ borderColor: "rgba(255,77,109,0.25)" }}
      >
        {/* Plan */}
                ...
                >
                    ...
                    <div className="flex items-center gap-1">
                      {d.locked ? (
                        <span
                          ...
                          style={{
                            color: d.remaining === 0 ? "var(--volt)" : "var(--destructive)",
                          }}
                        >
                          <Lock className="size-3" />{" "}
                          {d.remaining === 0 ? "Ready to unlock" : `Locked · ${d.remaining} left`}
                        </span>
                      ) : (
                        <span
                          className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider"
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    <input
                      ...
                      value={val}
                      onChange={(e) =>
                        setMasteryDraft((s) => ({ ...s, [d.id]: Number(e.target.value) }))
                      }
                      className="flex-1"
                    />
                    <span
                    ...
                    <button
                      onClick={() =>
                        dev.mutate({ domainMastery: [{ domainId: d.id, mastery: val }] })
                      }
                      disabled={dev.isPending}
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    {d.locked ? (
                      <>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const answered = Math.max(
                              0,
                              Math.min(SCORING.THRESHOLD_QUESTIONS, Number(e.target.value) || 0),
                            );
                            dev.mutate({
                              domainLock: [{ domainId: d.id, locked: true, answered }],
                            });
                          }}
                        ...
                        <button
                          onClick={() =>
                            dev.mutate({ domainLock: [{ domainId: d.id, locked: false }] })
                          }
                          disabled={dev.isPending}
                      ...
                      <button
                        onClick={() =>
                          dev.mutate({
                            domainLock: [{ domainId: d.id, locked: true, answered: 0 }],
                          })
                        }
                        disabled={dev.isPending}
                        className="ml-auto rounded-lg px-2 py-1 text-[10px] font-bold uppercase tracking-wider disabled:opacity-40"
        ...
        <section>
          <button
            ...
            onClick={() => {
              ...
              if (
                confirm(
                  "Reset demo account? This wipes all Daily 5 progress, sessions, streak, and mastery for your account.",
                )
              ) {
                resetDemo.mutate();
              }
          ...
          <button
            ...
            onClick={() => {
              ...
              if (
                confirm(
                  "Reset today's Daily 5? This clears today's answered questions so you can retake Daily 5. Mastery and momentum already earned today are kept.",
                )
              ) {
                resetDaily.mutate();
              }
  ...
  return (
    <section>
      ...
      <div
        ...
        style={{ background: "rgba(0,0,0,0.25)", border: "1px solid rgba(246,240,250,0.08)" }}
      >
            ...
            {(["auto", "win", "loss", "tie"] as const).map((r) => (
              <button
                ...
                style={{
                  ...
                  border:
                    result === r ? "1px solid var(--volt)" : "1px solid rgba(246,240,250,0.15)",
                }}
              >
                {r}
              </button>
      ...
      >
        ...
        {fakeStatus && (
          <div className="mt-2 text-[10px]" style={{ color: "var(--lavender)" }}>
            {fakeStatus}
          </div>
        )}
      </div>
...
function DevToggle({
  ...
  return (
    ...
    >
      <span className="text-xs font-bold" style={{ color: "var(--lavender)" }}>
        {label}
      </span>
      <span
