...
export function BonusUnlockModal({ open, domainId, domainLabel, onClose }: Props) {
  ...
  const [bonusSummary, setBonusSummary] = useState<{ correct: number; total: number; domainAnswered: number; domainCorrect: number; results: boolean[] } | null>(null);
    ...
    >
      {phase !== "chest" && (
        <button
          ...
          style={{ background: "rgba(0,0,0,0.5)", border: "1px solid rgba(246,240,250,0.2)", color: "var(--lavender)" }}
        >
          <X className="size-5" />
        </button>
      )}
        ...
        <div className="flex-1 flex flex-col">
          <header className="px-5 pt-4">
            <div className="mx-auto max-w-2xl">
              <div className="text-center text-[11px] font-bold uppercase tracking-widest" style={{ color: "rgba(246,240,250,0.7)" }}>
              ...
              <div className="mt-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(246,240,250,0.1)" }}>
                <div
                  ...
                  style={{ width: `${((qIndex + (selected !== null ? 1 : 0)) / 3) * 100}%`, background: "var(--volt)" }}
                />
              </div>
            </div>
          </header>
          ...
          <main className="flex-1 overflow-auto mx-auto max-w-2xl w-full px-5 pt-6 pb-10">
            <div className="rounded-2xl p-6 sm:p-8 animate-fade-up" style={{ background: "var(--lavender)", color: "var(--ink)" }}>
              <div
              ...
              {q.passage && (
                <p className="mt-4 text-sm leading-relaxed" style={{ color: "#3b2f57" }}>{q.passage}</p>
              )}
              <h1 className="mt-3 text-base sm:text-lg font-bold" style={{ color: "var(--ink)" }}>{q.question}</h1>
              ...
              <div className="mt-5 grid gap-2.5">
                {q.choices.map((choice, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-bold mt-0.5 shrink-0 opacity-80">{String.fromCharCode(65 + i)}</span>
